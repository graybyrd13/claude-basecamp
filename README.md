...
**Always On.** `claude-basecamp daemon install` runs Basecamp at login as a user-level service — launchd on macOS, systemd user unit on Linux, Task Scheduler on Windows — so checks, routines, and reflexes are alive without a terminal open. Due work catches up automatically after sleep. `claude-basecamp status` answers "is it running, is the service installed" from any shell; `daemon uninstall` removes it cleanly.
